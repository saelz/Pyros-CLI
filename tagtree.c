...
static void
NewTree(PyrosTag **tag, size_t length,size_t current,TagTree *tree,int depth){
	size_t i = current;
	...
	for (i = 0; i < length; i++) {
		if (tag[i]->par == current){
			...
			NewTree(tag,length,i,&tree->children[current_child],depth+1);
			tree->children[current_child].tag = tag[i]->tag;
			tree->children[current_child].is_Alias = tag[i]->isAlias;
			current_child++;
			tree->child_count++;
		}
	}
